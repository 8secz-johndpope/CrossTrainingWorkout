...
public class CommandLine {
     ...
     * ```
     */
    public fileprivate(set) var unparsedArguments: [String] = [String]()
...
class Localization {
    ...
    convenience init(inputFile : URL, delimiter : String, autocapitalize : Bool, bundle: String, table: String? = nil) {
        
        self.init()
        self.table = table
        // Load localization file
        self.processInputFromFile(file: inputFile, delimiter: delimiter, autocapitalize: autocapitalize, bundle: bundle)
    }
    
    
    func processInputFromFile(file : URL, delimiter : String, autocapitalize : Bool, bundle: String) {
    ...
    private func swiftLocalizationStaticVarFromLocalizationKey(key : String, variableName : String, baseTranslation : String, contentLevel : Int = 0) -> String {
        ...
        return TemplateFactory.templateForSwiftStaticVarWithName(name: self.variableName(string: variableName, lang: .Swift), key: key, bundle: bundleName, table: table, baseTranslation : baseTranslation, contentLevel: contentLevel)
    }
    ...
    private func swiftLocalizationFuncFromLocalizationKey(key : String, methodName : String, baseTranslation : String, methodSpecification : [SpecialCharacter], contentLevel : Int = 0) -> String {
        ...
        return TemplateFactory.templateForSwiftFuncWithName(name: self.variableName(string: methodName, lang: .Swift), key: key, bundle: bundleName, table: table, baseTranslation : baseTranslation, methodHeader: methodHeaderParams, params: methodParamsString, contentLevel: contentLevel)
    }
...
class Runtime {
    ...
    var bundleName: String = "Bundle.main"
    ...
    private func checkCLI() -> Bool {
        ...
        let bundleName = StringOption(shortFlag: "n", longFlag: "bundleName", required: false,
                                         helpMessage: "Optional | String | Name of the bundle | Defaults to \"Bundle.main\"")
        ...
        let cli = CommandLine()
        cli.addOptions(inputFilePath, bundleName, outputFilePath, outputLanguage, delimiter, autocapitalize, baseClassName, stringsTableName, customSuperclass)
        ...
        do {
            ...
            if let name = bundleName.value { self.bundleName = name }
    ...
    private func processOutput() {
        ...
        self.localizationCore = Localization(inputFile: self.localizationFilePathToRead, delimiter: self.localizationDelimiter, autocapitalize: self.localizationAutocapitalize, bundle: bundleName, table: self.localizationStringsTable)
...
class TemplateFactory {
    ...
    class func templateForSwiftStaticVarWithName(name : String, key : String, bundle: String, table: String?, baseTranslation : String, contentLevel : Int) -> String {
        ...
        return TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "/// Base translation: \(baseTranslation)\n"
            + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "public static var \(name) : String = NSLocalizedString(\"\(key)\", tableName: \(tableName), bundle: \(bundle), comment: \"\")\n"
    }
    
    
    class func templateForSwiftFuncWithName(name : String, key : String, bundle: String, table: String?, baseTranslation : String, methodHeader : String, params : String, contentLevel : Int) -> String {
        let tableName = table != nil ? "\"\(table!)\"" : "nil"
        return TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "/// Base translation: \(baseTranslation)\n"
             + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "public static func \(name)(\(methodHeader)) -> String {\n"
             + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel + 1) + "return String(format: NSLocalizedString(\"\(key)\", tableName: \(tableName), bundle: \(bundle), comment: \"\"), \(params))\n"
             + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "}\n"
    }
